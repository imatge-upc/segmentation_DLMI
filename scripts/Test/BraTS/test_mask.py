...
if __name__ == "__main__":
    ...
    dir_path = join(params[p.OUTPUT_PATH], 'LR_' + str(params[p.LR])+'_mask_' +
                    params[p.LOSS]+'_momentum0.99')

    logs_filepath = join(dir_path, 'results', filename + params[p.MODEL_TYPE] + '.txt')
    ...
    model, output_shape = BraTS_models.get_model(
        ...
        filepath_weights={'mask':join(dir_path, 'model_weights', params[p.MODEL_NAME] + '_mask_part.h5'),
                          'seg':join(dir_path, 'model_weights', params[p.MODEL_NAME] + '_seg_part.h5')}

    )
    model.summary()

    model.load_weights(join(dir_path, 'model_weights', params[p.MODEL_NAME] + '_seg_part.h5'), by_name=True)

    if params[p.MODEL_TYPE] == 'mask_load_seg':
        model = BraTS_models.compile(model, lr=params[p.LR], model_type='mask_seg', loss_name=params[p.LOSS])
    else:
    ...
    for sbj in subject_list_test:
        print(sbj.id)

    exit()
    if params[p.MODEL_TYPE] == 'mask':

        generator_test = dataset.data_generator_BraTS_mask(subject_list_test, mode='test')

    elif params[p.MODEL_TYPE] == 'mask_load_seg':
    ...
    for inputs, outputs, subject in generator_test:
        ...
        if params[p.MODEL_TYPE] == 'mask':
            ...
            #Metrics:
            dice_mask[n_sbj] = dice(predictions[:, :, :, 1].flatten(), labels[:, :, :, 1].flatten())


            print("Dice mask: " + str(dice_mask[n_sbj]))
            ...
            predictions_mask = np.floor(predictions_mask / np.max(predictions_mask, axis=3, keepdims=True)).astype(
                'int')
            predictions_seg_tmp = np.floor(
                predictions_seg_tmp / np.max(predictions_seg_tmp, axis=3, keepdims=True)).astype('int')
            ...
            # Saving images
            img = nib.Nifti1Image(predictions[1][0][:,:,:,0], subject.get_affine())
            nib.save(img, join(dir_path, 'results', subject.id + '_predictions_0.nii.gz'))

            img = nib.Nifti1Image(predictions[1][0][:, :, :, 1], subject.get_affine())
            nib.save(img, join(dir_path, 'results', subject.id + '_predictions_1.nii.gz'))

            img = nib.Nifti1Image(predictions[1][0][:, :, :, 2], subject.get_affine())
            nib.save(img, join(dir_path, 'results', subject.id + '_predictions_2.nii.gz'))
            ...
            dice_wt[n_sbj] = dice(np.sum(predictions_seg[:, :, :, 1:], axis=3).flatten(),
                                  np.sum(labels_seg[:, :, :, 1:], axis=3).flatten())
            dice_tc[n_sbj] = dice(np.sum(np.concatenate((predictions_seg[:, :, :, 1][:, :, :, np.newaxis],
                                                         predictions_seg[:, :, :, 3][:, :, :, np.newaxis]), axis=3),
                                         axis=3).flatten(),
                                  np.sum(np.concatenate((labels_seg[:, :, :, 1][:, :, :, np.newaxis],
                                                         labels_seg[:, :, :, 3][:, :, :, np.newaxis]), axis=3),
                                         axis=3).flatten())
