...
if __name__ == "__main__":
    ...
    for inputs, labels in generator_val:
        ...
        labels = preprocessing.one_hot_representation(labels, params[p.N_CLASSES])
        ...
        #
        print(predictions_resized.shape)
        print(labels.shape)
        ...
        # print("Count 2: " + str(np.sum(predictions[0,:,:,:,2]*labels[0,:,:,:,2])) +' '+ str(np.sum(predictions[0,:,:,:,2])) + ' ' + str(np.sum(labels[0,:,:,:,2])))
        # print("Count 3: " + str(np.sum(predictions[0,:,:,:,3]*labels[0,:,:,:,3])) +' '+ str(np.sum(predictions[0,:,:,:,3])) + ' ' + str(np.sum(labels[0,:,:,:,3])))
        #
        #
