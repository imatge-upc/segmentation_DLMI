...
if __name__ == "__main__":
    ...
    for i_CAE in range(5):
        CAE_list.append({'n_filters': 8, 'train_flag': False, 'weights_filename': None})
    ...
    model = iSeg_models.compile_ACNN(model, lr=params[p.LR])
    ...
    if params[p.SAMPLING_SCHEME] == 'whole':
        ...
        generator_train = dataset.data_generator_iSeg_ACNN(subject_list_train, mode='train', normalize_bool=True)
        generator_val = dataset.data_generator_iSeg_ACNN(subject_list_validation, mode='validation', normalize_bool=True)
    else:
