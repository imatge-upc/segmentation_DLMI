...
if __name__ == "__main__":
    ...
    dir_path = join(params[p.OUTPUT_PATH], 'LR_' + str(params[p.LR])+'_mask_seg_' +
                    params[p.LOSS]+'_momentum0.99_BNnormal')
    ...
    if params[p.N_SUBJECTS_TRAIN] is not None:
        steps_per_epoch = params[p.N_SUBJECTS_TRAIN]
    else:
    ...
    if params[p.N_SUBJECTS_VALIDATION] is not None:
        validation_steps = params[p.N_SUBJECTS_VALIDATION]
    else:
    ...
    elif params[p.MODEL_TYPE] == 'mask_seg':
        generator_train = dataset.data_generator_BraTS_mask_seg(subject_list_train, mode='train',
                                                                sample_weights_bool=params[p.SAMPLE_WEIGHTS_BOOL],
                                                                class_weights=class_weights,
                                                                n_iterations = steps_per_epoch
                                                                )
        generator_val = dataset.data_generator_BraTS_mask_seg(subject_list_validation, mode='validation',
                                                              n_iterations=validation_steps,
                                                              sample_weights_bool=params[p.SAMPLE_WEIGHTS_BOOL],
                                                              class_weights=class_weights
                                                              )
    elif params[p.MODEL_TYPE] == 'seg':
        generator_train = dataset.data_generator_BraTS_seg(subject_list_train, mode='train',
                                                                sample_weights_bool=params[p.SAMPLE_WEIGHTS_BOOL],
                                                                class_weights=class_weights,
                                                                )
        generator_val = dataset.data_generator_BraTS_seg(subject_list_validation, mode='validation',
                                                         sample_weights_bool=params[p.SAMPLE_WEIGHTS_BOOL])
    else:
