...
if __name__ == "__main__":
    ...
    dir_path = join(params[p.OUTPUT_PATH], 'LR_' + str(params[p.LR]) + '_dice_DA_' + str(params[p.DATA_AUGMENTATION_FLAG]) +
                    '_BN_' + str(params[p.BN_LAST]) + '_shortcut_' + str(params[p.SHORTCUT_INPUT])
                    + '_mask_' + str(params[p.BOOL_MASK]) + '_woBN')
    ...
    # params[p.LR] = params[p.LR]/1000
    # model.load_weights(weights_filepath[:-3]+'_epoch2.h5', by_name=True)
    model = SegmentationModels.compile(model, lr=params[p.LR],num_classes=params[p.N_CLASSES])#, loss_name='dice',optimizer_name='SGD')
